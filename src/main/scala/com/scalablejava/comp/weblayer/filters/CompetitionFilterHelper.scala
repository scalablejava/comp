...
class CompetitionFilterHelper extends CompBaseFilter {
  ...
  def comboDays(number: Int): String = {
    var html = ""
    html += "<select style='width:150px;' name='day" + number + "' id='day" + number + "'>"
  ...
  def comboVenue(competitionId: Long, number: Int): String = {
    var html = ""
    html += "<select style='width:200px;' name='venue" + number + "' id='venue" + number + "'>"
  ...
  def settingPerMatchHtml(competitionId: Long, numberOfTeams: Int, matchesBetweenTeams: Int): String = {
    ...
    for (i<-1 to (numberOfTeams/2)) {
      ...
      html += "<div class=\"grid_4\" style='text-align:center;'>" + comboVenue(competitionId, i) + "</div>"
      html += "<div class=\"grid_3\" style='text-align:center;'>" + comboDays(i) + "</div>"
      html += "<div class=\"grid_3\" style='text-align:center;'><input type='text' name='startTime" + i + "' id='startTime" + i + "'/></div>"
      html += "<div class=\"grid_3\" style='text-align:center;'><input type='text' name='endTime" + i + "' id='endTime" + i + "'/></div>"
    }
    return html
  }
  def settingSemifinalRoundHtml(competitionId: Long, numberOfTeams: Int, matchesBetweenTeams: Int): String = {
    var html = ""
  ...
  def settingFinalRoundHtml(competitionId: Long, numberOfTeams: Int, matchesBetweenTeams: Int): String = {
    var html = ""
